...
<h2><?php  echo sprintf(__('User %s'), "<u>{$user['User']['login']}</u>"); ?></h2>
<?php if (empty($actions)) { ?>
    <dl><?php $i = 0; $class = ' class="altrow"';?>
<?php if ($user['User']['disable']) { ?>
        <dt<?php if ($i % 2 == 0) echo $class;?>><?php echo $this->Html->image("icons/error.png", array('title' => __('Warning'))); ?></dt>
        <dd<?php if ($i++ % 2 == 0) echo $class;?>>
            <?php
                    echo "<strong>".__('Account disabled')."</strong>";
            ?>
            &nbsp;
        </dd>
<?php } ?>        
...
<div class="actions">
    <ul>
<?php if (!empty($actions)) { ?>
        <li class="icon user"><?php echo $this->Html->link(__('View user'), array('action'=>'view', $user['User']['id'])); ?></li>
<?php } ?>
        <li class="icon user_edit"><?php echo $this->Html->link(__('Edit user'), array('action'=>'edit', $user['User']['id'])); ?></li>
<?php if (empty($actions)) { ?>
        <li class="icon lock"><?php echo $this->Html->link(__('View allowed & denied actions'), array('action'=>'view', "{$user['User']['id']}/actions")); ?></li>
        <li class="icon cross"><?php echo $this->Html->link(__('Delete user'), array('action'=>'delete', $user['User']['id']), null, sprintf(__('Are you sure you want to delete user \'%s\'?'), $user['User']['login'])); ?></li>
<?php } ?>
    </ul>
</div>
...
<p style="clear: both"></p>
    ...
    <div class="actions">
        <ul>
            ...
            <li class="icon accept"><?php echo $this->Html->link(__('Hide this view'), array('action'=>'view', $user['User']['id'])); ?></li>
        </ul>
    </div>
<?php } ?>

<div class="related">
    <h3><?php echo sprintf(__('Rules applied to user %s'), "{$user['User']['login']}");?></h3>
    <?php if (!empty($rules)):?>
